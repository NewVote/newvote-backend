...
let cfMongoUrl = (function () {
    if (appEnv.getService('mean-mongo')) {
        let mongoCreds = appEnv.getService('mean-mongo').credentials
        return mongoCreds.uri || mongoCreds.url
    } else {
        throw new Error(
            'No service names "mean-mongo" bound to the application.',
        )
    }
})()

let getCred = function (serviceName, credProp) {
...
module.exports = {
    ...
    mailer: {
        ...
        options: {
            service:
                getCred('mean-mail', 'service') || 'MAILER_SERVICE_PROVIDER',
            auth: {
