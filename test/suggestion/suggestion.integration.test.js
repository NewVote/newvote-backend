...
describe('/api/suggestions POST routes', async function () {
    const request = chai.request(server).keepOpen();
    const ignoredFields = ['user', 'parent', 'parentType', 'organizations', '_id'];
    let newSuggestion;
    let savedOrg
    ...
    afterEach(async function () {
        Suggestion.findByIdAndDelete(newSuggestion._id)
            .then((res) => {
                if (!res) return false;
...
describe('/api/suggestions/:suggestionId PUT', async function () {
    ...
    afterEach(async function () {
        return await Suggestion.findOne({ _id: suggestionId })
            .then((doc) => {
    ...
    it('unauthenticated user cannot update Suggestion', async function () {
        ...
        return newDbSuggestion.description.should.not.equal(alteredSuggestion.description);   
    });
...
describe('/api/suggestions/:suggestionId DELETE', async function () {
    ...
    it('unauthenticated user cannot delete Suggestion', async function () {
        ...
        // Second test to make sure DB has not been updated
        const proposalExists = await Suggestion.findOne({ _id: originalSuggestion._id });
    ...
    it('admin can delete Suggestion', async function () {
        ...
        res.should.have.status(200);
        // Second test to make sure DB has not been updated
        return Suggestion.findOne({ _id: originalSuggestion._id })
            .then((doc) => {
