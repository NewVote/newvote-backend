...
describe('/api/media POST routes', async function () {
    const request = chai.request(server).keepOpen();
    const ignoredFields = ['issues', 'solutions', 'proposals', 'votes', 'organizations', 'url'];
    ...
    before(async function () {
        ...
        Organization.findOne()
            .then(res => savedOrg = res);
        Issue.findOne()
            .then(res => savedIssues = res);
    ...
    afterEach(async function () {
        Media.findByIdAndDelete(newMedia._id)
            .then((res) => {
                if (!res) return false;
                newMedia = generateDoc(Media, ignoredFields);
                newMedia.organizations = savedOrg;
...
describe('/api/media/:mediaId GET', async function () {
    ...
    before(async function () {
        return await Media.findOne()
            .then((mediaItem) => newMedia = mediaItem)
    })
...
describe('/api/media/:newMedia.id PUT', async function () {
    ...
    before(async function () {
        return await Media.findOne()
            .then((mediaItem) => newMedia = JSON.parse(JSON.stringify(mediaItem)));
    })

    // Reset the original media after each test
    afterEach(async function () {
        return await Media.findOne({ _id: newMedia._id })
            .then((doc) => {
...
describe('/api/media/:mediaId DELETE', async function () {
    ...
    it('admin can delete Media', async function () {
        ...
        res.should.have.status(200);
        // Second test to make sure DB has not been updated
        return Media.findOne({ _id: newMedia._id })
            .then((doc) => {
