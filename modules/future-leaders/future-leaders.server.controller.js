...
exports.update = function (req, res) {
    ...
    const findUserPromise = User.findOne({ email }).then((user) => {
        if (user) throw 'User exists on database'
        return false
    })

    const doesNewLeaderExist = FutureLeader.findOne({ email }).then(
        (leader) => {
    ...
    const doesOrganizationExistOnFutureLeader = FutureLeader.findOne({
    ...
    return Promise.all([
        findUserPromise,
        doesNewLeaderExist,
        doesOrganizationExistOnFutureLeader,
        findOrganizationPromise,
    ])
        .then((promises) => {
...
function sendVerificationCodeViaEmail(req, res, user) {
    ...
    if (user.emailDelivered)
        return res.status(200).send({ message: 'Leader Successfully Added' })
