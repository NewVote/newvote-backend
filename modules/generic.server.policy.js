...
async function canAccessOrganization(req, object) {
    ...
    const { owner, moderators } = await Organization.findOne({
        _id: reqOrg._id,
    }).populate('owner')
    ...
    const rep = await Rep.findOne({ _id: id, organizations: reqOrg._id })
    ...
    if (method === 'put') {
        if (!isOwner && !isModerator && !isCreator)
            throw 'User does not have access to that route'
        // block access to organization editing
        if (!isOwner && collection && collection.name === 'organizations')
            throw 'An error occoured while validating your credentials'
        return true
    }
