...
module.exports = function (app) {
    ...
    app.route('/api/auth/signup/:verificationCode?').post(users.signup)
    app.route('/api/auth/signin').post(
        passport.authenticate('local'),
        users.signin,
    )
    app.route('/api/auth/signout').get(users.signout)

    app.route('/api/auth/check-status').get(users.checkAuthStatus)
    ...
    app.route('/api/auth/facebook/callback').get(
    ...
    app.route('/api/auth/linkedin/callback').get(
        users.oauthCallback('linkedin'),
    )
